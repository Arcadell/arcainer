...
export const useContainerStore = defineStore("containerData", {
    ...
    actions: {
        ...
        async getContainerLogs(id: string) {
            try {
                const auth = useAuthStore();
                const response = await fetch((import.meta.env.DEV ? import.meta.env.VITE_API_URL : '') + '/api/container/logs?id=' + id, {
                    method: 'GET',
                    headers: new Headers({
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + await auth.getAccessToken(),
                ...
                const okResponse = await response.json() as string;
                ...
                return { data: 'Error fetching logs', error: e.message };
            }
        },
    }
})